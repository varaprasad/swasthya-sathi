...
class SynthesizerSelection(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """SynthesizerSelection - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] ([str]): This parameter is optional. If specified, it overrides the synthesizer list from the project settings.  A list of synthesizer vendor names like `\"GOOGLE\"`. When using custom synthesizer profiles, the profile name is attached as a suffix separated by a dash, e.g. `\"GOOGLE-profile\"`. Alternatively, the profile token can be used directly (without the vendor name).  The first synthesizer in the list has the highest priority. Additional synthesizers are used as a fallback (in order) in case a service is currently unreachable.  The following synthesizer vendors are currently available:    - `GOOGLE`   - `MICROSOFT`   - `IBM`   - `AMAZON`   - `NUANCE`.  # noqa: E501

        Keyword Args:
            value ([str]): This parameter is optional. If specified, it overrides the synthesizer list from the project settings.  A list of synthesizer vendor names like `\"GOOGLE\"`. When using custom synthesizer profiles, the profile name is attached as a suffix separated by a dash, e.g. `\"GOOGLE-profile\"`. Alternatively, the profile token can be used directly (without the vendor name).  The first synthesizer in the list has the highest priority. Additional synthesizers are used as a fallback (in order) in case a service is currently unreachable.  The following synthesizer vendors are currently available:    - `GOOGLE`   - `MICROSOFT`   - `IBM`   - `AMAZON`   - `NUANCE`.  # noqa: E501
