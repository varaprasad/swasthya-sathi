...
class TranscriberSelection(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """TranscriberSelection - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] ([str]): This parameter is optional. If specified, it overrides the transcriber list from the project settings.  A list of transcriber vendor names like `\"GOOGLE\"`. When using custom transcriber profiles, the profile name is attached as a suffix separated by a dash, e.g. `\"GOOGLE-profile\"`. Alternatively, the profile token can be used directly (without the vendor name).  The first transcriber in the list has the highest priority. Additional transcribers are used as a fallback (in order) in case a service is currently unreachable.  The following transcriber vendors are currently available:    - `GOOGLE`   - `MICROSOFT`   - `IBM`.  # noqa: E501

        Keyword Args:
            value ([str]): This parameter is optional. If specified, it overrides the transcriber list from the project settings.  A list of transcriber vendor names like `\"GOOGLE\"`. When using custom transcriber profiles, the profile name is attached as a suffix separated by a dash, e.g. `\"GOOGLE-profile\"`. Alternatively, the profile token can be used directly (without the vendor name).  The first transcriber in the list has the highest priority. Additional transcribers are used as a fallback (in order) in case a service is currently unreachable.  The following transcriber vendors are currently available:    - `GOOGLE`   - `MICROSOFT`   - `IBM`.  # noqa: E501
